...
import { Camera, Type, Flame, Clock, Users, Heart, X, RefreshCw, Volume2, Sparkles, ChevronRight, ArrowLeft, ChefHat, Filter } from "lucide-react";
...
export function FridgeTab() {
  ...
  const handleRegenerate = () => {
    setIsGenerating(true);
    setTimeout(() => {
      setRecipes([...SAMPLE_RECIPES].sort(() => Math.random() - 0.5));
      setIsGenerating(false);
    }, 1000);
  };
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div className="bg-white rounded-2xl p-4 border border-gray-100 mb-4">
            ...
            <div className="flex gap-2 mb-3">
              {[{ label: "Protein", value: selectedRecipe.protein, color: "#84cc16" }, { label: "Carbs", value: selectedRecipe.carbs, color: "#f97316" }, { label: "Fat", value: selectedRecipe.fat, color: "#a855f7" }].map((n) => (
                <div key={n.label} className="flex-1 py-2 rounded-xl text-center" style={{ backgroundColor: `${n.color}15` }}>
                  <span className="text-xs font-bold" style={{ color: n.color }}>{n.value}g</span>
              ...
              {recipes.map((recipe) => (
                <div key={recipe.id} onClick={() => setSelectedRecipe(recipe)} className="bg-white rounded-3xl overflow-hidden border-2 border-gray-100 shadow-lg cursor-pointer">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      {[{ label: "P", value: `${recipe.protein}g`, color: "#84cc16" }, { label: "C", value: `${recipe.carbs}g`, color: "#f97316" }, { label: "F", value: `${recipe.fat}g`, color: "#a855f7" }].map((n) => (
                        <div key={n.label} className="flex-1 py-2 rounded-xl text-center" style={{ backgroundColor: `${n.color}15` }}>
