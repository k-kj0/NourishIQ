...
import { 
  ...
  Sparkles, Plane, ShoppingBasket, User, X, Plus, Minus, Camera, Check
} from "lucide-react";
...
export default function NourishIQApp() {
  ...
  const [groceryList, setGroceryList] = useState<{ name: string; checked: boolean }[]>([
    ...
    { name: "Garlic, minced", checked: false }
  ]);
    ...
    return (
      <div className="min-h-screen max-w-md mx-auto flex flex-col justify-between px-6 py-8 bg-[#FCFBF7] shadow-2xl">
        ...
        <div className="flex-1 flex flex-col justify-center my-8 text-center">
          ...
          {profile.step === 1 && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-500 max-w-xs mx-auto leading-relaxed bg-white border p-4 rounded-2xl">
                Personalised meal plans, plate scans, travel cuisine, cravings, and DIY apothecary — all in one cute little app.
              </p>
            </div>
          )}
          ...
          {profile.step === 2 && (
            <div className="space-y-4">
              ...
              <input type="text" value={profile.name} onChange={e => setProfile(p => ({ ...p, name: e.target.value }))} placeholder="Your name" className="w-full max-w-xs mx-auto border bg-white rounded-full px-5 py-3 text-xs shadow-inner text-center font-bold" />
            </div>
          )}

          {profile.step >= 3 && profile.step <= 9 && (
            <div className="space-y-2 max-w-xs mx-auto w-full">
              <h3 className="text-base font-bold text-gray-800 mb-2">Configure Profile Metric Layer</h3>
              {["Lose fat", "Build muscle", "Eat cleaner", "Omnivore", "Vegetarian", "Vegan"].map((g) => (
                <button key={g} type="button" onClick={() => { setProfile(p => ({ ...p, goal: g })); handleNext(); }} className="w-full bg-white border p-3 rounded-xl font-bold text-xs text-gray-700 hover:border-green-600 transition-all mb-2">{g}</button>
              ))}
            </div>
          )}
          ...
          {profile.step === 10 && (
            <div className="space-y-3">
              <h3 className="text-base font-bold text-gray-800">Appliances you have 🍳</h3>
              <div className="flex flex-wrap gap-2 justify-center max-w-xs mx-auto">
                {["Stovetop", "Oven", "Air fryer", "Microwave", "Blender"].map(ap => (
        ...
        <button type="button" onClick={handleNext} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3.5 rounded-full text-xs transition-all">
          {profile.step === 10 ? "Finish ✓" : "Continue"}
        </button>
      </div>
    );
  }
        ...
        {activeTab === "Home" && (
          <div className="p-4 animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="w-8 h-8 rounded-full bg-white border flex items-center justify-center text-xs cursor-pointer shadow-sm">🔔</span>
            </div>
            ...
            <div className="space-y-3 mb-6">
              {DATA_MEALS.map((meal) => (
                <div key={meal.id} onClick={() => { setSelectedMeal(meal); setShowRecipeSheet(true); }} className="bg-white rounded-2xl border flex items-center p-2.5 gap-3 cursor-pointer group hover:border-green-200 transition-colors">
            ...
            {/* FULLY FUNCTIONAL PLATE SCANNER ENGINE */}
            <div className="bg-white border border-neutral-100 rounded-2xl p-4 shadow-sm mb-5 relative overflow-hidden">
              <div className="flex justify-between items-center mb-1">
                ...
                {scanState !== "scanning" && (
                  <button type="button" onClick={scanState === "done" ? () => setScanState("idle") : startScanModule} className="text-[10px] bg-green-600 text-white font-black px-2.5 py-1 rounded-full shadow-sm">
                    {scanState === "done" ? "Reset" : "Scan Plate"}
                  </button>
                )}
              </div>
              ...
              {scanState === "idle" && (
                <div onClick={startScanModule} className="bg-neutral-50 py-6 rounded-xl border border-dashed border-gray-200 text-center text-gray-400 text-xs font-bold cursor-pointer hover:bg-neutral-100 transition-colors flex flex-col items-center justify-center gap-1">
              ...
              {scanState === "scanning" && (
                <div className="bg-neutral-900 text-white py-6 rounded-xl relative overflow-hidden text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-xs font-black tracking-widest text-green-400 animate-pulse">ANALYZING MEAL BOUNDARIES...</p>
                </div>
              )}

              {scanState === "done" && (
                <div className="bg-green-50/70 border border-green-200 p-3 rounded-xl text-left animate-fade-in">
                  <p className="text-xs font-black text-green-700 flex items-center gap-1">✨ Plate Scan Complete</p>
                  <div className="mt-2 space-y-1 text-[11px] text-gray-700 font-medium">
                    <p>• <span className="font-bold">Detected:</span> Avocado Toast with Sunny Egg</p>
                    <p>• <span className="font-bold">Nutrition:</span> <span className="text-orange-600 font-bold">320 kcal</span> | <span className="text-green-700 font-bold">12g Protein</span></p>
        ...
        {activeTab === "Craver" && (
          <div className="p-4 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-3">
              {["Spaghetti", "Sushi", "Tacos", "Ramen", "Pizza", "Buddha Bowl"].map((pick, i) => (
                <div key={i} onClick={() => { setCraverInput(pick); setCraverResult(`Perfect! Generating options layouts for ${pick}...`); }} className="bg-white border rounded-2xl p-4 cursor-pointer shadow-sm text-left hover:border-pink-300 transition-colors">
                  <p className="font-bold text-sm text-gray-800">{pick}</p>
        ...
        {activeTab === "Me" && (
          <div className="p-4 animate-fade-in">
            ...
            <div className="bg-white border rounded-2xl p-4 shadow-sm text-center">
              <p className="text-xs font-bold text-gray-700">User Profile Framework Connected</p>
              <button type="button" onClick={() => { setIsOnboarded(false); setProfile({ name: "", step: 1, goal: "Eat cleaner", diet: "Omnivore" }); }} className="mt-4 w-full bg-neutral-900 text-white font-bold py-2.5 rounded-xl text-xs shadow-sm">Restart Quiz Flow</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex flex-col justify-end" onClick={() => setShowRecipeSheet(false)}>
          <div className="bg-white rounded-t-3xl max-w-md mx-auto w-full p-4 overflow-y-auto max-h-[75vh]" onClick={e => e.stopPropagation()}>
            ...
            <button type="button" onClick={() => {
              ...
              setGroceryToast(`Added ${selectedMeal.name} items`);
          ...
          <div className="bg-white rounded-t-3xl max-w-md mx-auto w-full p-4 max-h-[65vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {APOTHECARY_ITEMS.map(ap => (
                <div key={ap.id} className="p-3 rounded-xl border text-left bg-neutral-50 shadow-inner" style={{ borderLeft: `4px solid ${ap.color}` }}>
